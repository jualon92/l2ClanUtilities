<script>
    ...
    import {  getPedidos, setPedidoEstado} from "../db"
    import { addPedido, addPuntaje, getDataByMail, getDataByName, deletePedido} from "../db";
...
import { get } from "svelte/store";
    ...
    const cambiarEtapa = async (pedido) => {
        ...
        const nuevaEtapa = pedido.etapa == "finalizado" ? "pendiente" : "finalizado"
...
<div class="contenedor-pedidos">
    ...
    {:else}
    <div>  <table class="table table-hover table-responsive " in:fade={{duration:500}}>
        <thead>
          <tr>
        ...
        <tbody> 
            {#each $Pedidos as pedido}
                   <tr class="table-active">
                    <th scope="row">{pedido.item} 
                        <div>{pedido.fecha}</div>
                    </th>
                    <td>{pedido.personaje}</td>
                    <td> 
                       
                        <button class="badge   badge-estado btn-lg rounded-pill table-striped  {pedido.etapa == "finalizado" ? "bg-warning" : "bg-success"}"    >{pedido.etapa}  </button> </td>
                    
                    <td> <button class="btn btn-outline-warning" style="width:49px;height:38px"  on:click={ () => cambiarEtapa(pedido)}>✔</button>  
                     <button class="btn btn-danger" on:click|preventDefault={ () => borrarPedido(pedido)}> 
                        <svg
